...
const MCQViewer = () => {
  ...
  return <div className="space-y-8 mx-0 my-0 px-0 py-0">
      ...
      <Card className="shadow-none border-0 bg-white/90 rounded-xl transition-all duration-300 hover:shadow-xl border hover:border-blue-200 relative pb-24">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                {currentMCQ.chapter && <Badge variant="outline">{currentMCQ.chapter}</Badge>}
            ...
            <Button onClick={() => deleteMCQ(currentMCQ.id)} variant="ghost" size="icon" className="hover:bg-red-50" disabled={deleting === currentMCQ.id} title="Delete MCQ">
              {deleting === currentMCQ.id ? <svg className="w-5 h-5 animate-spin text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg> : <Trash2 className="w-5 h-5 text-red-500" />}
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-3">
            ...
            return <Button key={index} onClick={() => handleAnswerSelect(index)} variant={isSelected ? "default" : "outline"} className={`justify-start text-left h-auto py-4 px-5 transition-all duration-300 rounded-lg border text-base font-medium hover:bg-gray-50 ${showCorrectness ? isCorrect ? 'bg-green-50 border-green-500 text-green-800' : isSelected ? 'bg-red-50 border-red-500 text-red-800' : '' : isSelected ? 'bg-blue-50 border-blue-500' : ''} ${showAnswer ? 'pointer-events-none' : 'cursor-pointer'}`} disabled={showAnswer}>
          ...
          {showAnswer && <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6 border border-blue-100">
        ...
        <div className="absolute left-0 right-0 -bottom-6 z-40 flex justify-end bg-white/90 border-t border-blue-100 py-4 px-4 shadow-lg rounded-b-xl">
          <div className="w-full max-w-2xl flex gap-3 justify-end mx-auto">
            {!showAnswer ? <Button onClick={checkAnswer} disabled={selectedAnswer === null} variant="default" size="lg" className="rounded-lg flex items-center gap-2">
                <CheckCircle className="w-5 h-5" />
                Check Answer
              </Button> : <>
                <Button onClick={previousQuestion} disabled={currentIndex === 0} variant="outline" size="lg" className="rounded-lg flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
                  Previous
                </Button>
                <Button onClick={nextQuestion} variant="default" size="lg" className="rounded-lg flex items-center gap-2">
